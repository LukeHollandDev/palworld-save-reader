...
func TestRunValidatesModeSpecificFlags(t *testing.T) {
	tests := [][]string{
		...
		{"--list-presets", "--explain"},
		{"--full", "--schema", "projection.json", "Level.sav"},
		{"--schema", "projection.json"},
		// --decode-raw changes how --full renders byte arrays and means nothing
		// to the projection modes, so pairing it with them is a usage error
		// rather than a silently ignored flag.
		{"--decode-raw", "Level.sav"},
		{"--preset", "player-details", "--decode-raw", "Level.sav"},
		{"--schema", "projection.json", "--decode-raw", "Level.sav"},
		{"--list-presets", "--decode-raw"},
	}
...
// TestRunResolvesPresetsByNameAlone covers the removal of --game-version: a
// preset is selected by name, an unrecognised name is a usage error, and the
// retired flag is rejected by the flag set itself.
func TestRunResolvesPresetsByNameAlone(t *testing.T) {
	var stdout, stderr bytes.Buffer
	status := run([]string{"--preset", "not-a-bundled-preset", "Level.sav"}, &stdout, &stderr)
	if status != 2 {
		t.Fatalf("status = %d, want 2; stderr = %q", status, stderr.String())
	}
	if !strings.Contains(stderr.String(), "is not bundled") {
		t.Fatalf("stderr = %q", stderr.String())
	}

	stdout.Reset()
	stderr.Reset()
	arguments := []string{"--preset", "player-details", "--game-version", "1.0.1.100619", "Level.sav"}
	if status := run(arguments, &stdout, &stderr); status != 2 {
		t.Fatalf("--game-version status = %d, want 2", status)
	}
}
...
func syntheticSave(t *testing.T) []byte {
	...
	// Wrap the payload in the PlM/0x31 container Palworld 1.X writes. The body
	// is a single Oodle block header, 0x4c for a raw block and 0x0a for the
	// Mermaid decoder, followed by one quantum stored verbatim.
	body := append([]byte{0x4c, 0x0a}, gvas.data...)

	var container syntheticArchive
	container.u32(uint32(len(gvas.data)))
	container.u32(uint32(len(body)))
	container.data = append(container.data, "PlM"...)
	container.u8(0x31)
	container.data = append(container.data, body...)
