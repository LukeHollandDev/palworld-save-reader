...
func TestPropertyTagLayoutAndFallback(t *testing.T) {
	...
	save, err := Parse(testGVAS(properties.bytes(), []byte{0, 0, 0, 0}))
	if err != nil {
		t.Fatal(err)
	}
...
func TestLazyCollectionsWithRemovedValues(t *testing.T) {
	...
	save, err := Parse(testGVAS(properties.bytes(), nil))
	if err != nil {
		t.Fatal(err)
	}
...
func TestLimitsPropagateInsteadOfBecomingRaw(t *testing.T) {
	...
	_, err := ParseWithOptions(testGVAS(outer.bytes(), nil), Options{MaxDepth: 1})
	var limitErr *LimitError
	if !errors.As(err, &limitErr) {
		t.Fatalf("error = %v, want LimitError", err)
	}

	_, err = ParseWithOptions(testGVAS(outer.bytes(), nil), Options{MaxPathBytes: 4})
...
func TestMalformedFStringAndPropertySize(t *testing.T) {
	...
	if _, err := Parse(testGVAS(malformed.bytes(), nil)); err == nil {
	...
	property.u32(0)
	property.u8(0)
	if _, err := Parse(testGVAS(property.bytes(), nil)); err == nil {
...
func FuzzParse(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		_, _ = ParseWithOptions(data, Options{
...
// propertyValue asserts that the named property decoded to T, so a test can
// state the encoding it expects on one line and fail with the type it got.
