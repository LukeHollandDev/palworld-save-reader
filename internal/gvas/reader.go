...
// Kind names the subject, and the message carries no package prefix because
// savefile raises this same type for the byte limits it owns.
...
func (err *LimitError) Error() string {
	return fmt.Sprintf("%s %d exceeds limit %d", err.Kind, err.Value, err.Limit)
}
...
func (r *archiveReader) take(n int) ([]byte, error) {
	if n < 0 || n > r.remaining() {
		return nil, fmt.Errorf("gvas: unexpected EOF at offset %d reading %d bytes (%d remain)", r.offset(), n, r.remaining())
	}
...
func (r *archiveReader) optionalGUID() (*GUID, error) {
	...
	if present != 1 {
		return nil, fmt.Errorf("gvas: invalid optional GUID marker %d at offset %d", present, r.offset()-1)
	}
	value, err := r.guid()
	if err != nil {
		return nil, err
...
func (r *archiveReader) fstring() (string, error) {
	...
	if count > 0 {
		...
		if size > int64(r.remaining()) {
			return "", fmt.Errorf("gvas: invalid FString length %d at offset %d", count, start)
		}
		...
		if len(value) == 0 || value[len(value)-1] != 0 {
			return "", fmt.Errorf("gvas: unterminated FString at offset %d", start)
		}
		return string(value[:len(value)-1]), nil
	}
	if count == math.MinInt32 {
		return "", fmt.Errorf("gvas: invalid UTF-16 FString length at offset %d", start)
	}
	units := int64(-count)
	if units*2 > int64(r.remaining()) {
		return "", fmt.Errorf("gvas: invalid UTF-16 FString length %d at offset %d", count, start)
	}
	...
	if len(value) < 2 || value[len(value)-2] != 0 || value[len(value)-1] != 0 {
		return "", fmt.Errorf("gvas: unterminated UTF-16 FString at offset %d", start)
	}
...
func validateCount(cfg *decodeConfig, kind string, count uint32, remaining, minimum int) error {
	...
	if uint64(count)*uint64(minimum) > uint64(remaining) {
		return fmt.Errorf("gvas: %s count %d cannot fit in %d bytes", kind, count, remaining)
	}
	return nil
}
