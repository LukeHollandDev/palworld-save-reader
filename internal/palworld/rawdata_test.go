...
func TestClassifyRawData(t *testing.T) {
	const slots = ".worldSaveData.ItemContainerSaveData.Value.Slots.Slots.RawData"
	if got := ClassifyRawData(slots); got != RawDataItemSlot {
		t.Errorf("ClassifyRawData(item slots) = %v, want %v", got, RawDataItemSlot)
	}
	for _, path := range []string{
		"",
		".worldSaveData.CharacterSaveParameterMap.Value.RawData",
		".worldSaveData.ItemContainerSaveData.Value.Slots.Slots",
	} {
		if got := ClassifyRawData(path); got != RawDataUnknown {
			t.Errorf("ClassifyRawData(%q) = %v, want unknown", path, got)
		}
	}
	if got := RawDataItemSlot.String(); got != "itemSlot" {
		t.Errorf("RawDataItemSlot.String() = %q", got)
	}
	if got := RawDataUnknown.String(); got != "unknown" {
		t.Errorf("RawDataUnknown.String() = %q", got)
	}
}
