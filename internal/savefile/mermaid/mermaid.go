...
func Decompress(src []byte, outputSize int) ([]byte, error) {
	if outputSize < 0 {
		return nil, fmt.Errorf("mermaid: negative output size")
	}
	...
	for dstAt < len(dst) {
		if dstAt%oodleQuantumSize == 0 {
			if len(src)-srcAt < 2 {
				return nil, fmt.Errorf("mermaid: truncated Oodle block header at %#x", srcAt)
			}
			first, second := src[srcAt], src[srcAt+1]
			srcAt += 2
			if first&0x0f != 0x0c || (first>>4)&3 != 0 {
				return nil, fmt.Errorf("mermaid: invalid Oodle block header %#x", first)
			}
			header = oodleHeader{
				decoder:   second & 0x7f,
			...
			if header.decoder != oodleDecoderMermaid {
				return nil, fmt.Errorf("mermaid: unsupported Oodle decoder %d (want Mermaid/10)", header.decoder)
			}
		}
		...
		if header.raw {
			if rawCount > len(src)-srcAt {
				return nil, fmt.Errorf("mermaid: truncated raw Oodle quantum")
			}
		...
		if len(src)-srcAt < 3 {
			return nil, fmt.Errorf("mermaid: truncated Oodle quantum header")
		}
		...
		if stored == 0x3ffff {
			...
			if special != 1 || len(src)-srcAt < 1 {
				return nil, fmt.Errorf("mermaid: unsupported Oodle special quantum %d", special)
			}
			value := src[srcAt]
			srcAt++
		...
		if header.checksums {
			...
			return nil, fmt.Errorf("mermaid: checksummed Oodle quanta are unsupported")
		}
		if compressed > len(src)-srcAt || compressed > rawCount {
			return nil, fmt.Errorf("mermaid: invalid Oodle quantum size %d -> %d", compressed, rawCount)
		}
		...
		if compressed == rawCount {
			...
			if err != nil {
				return nil, fmt.Errorf("mermaid: quantum at output %#x: %w", dstAt, err)
			}
			if used != len(quantum) {
				return nil, fmt.Errorf("mermaid: quantum consumed %d/%d bytes", used, len(quantum))
			}
		}
		srcAt += compressed
		dstAt += rawCount
	...
	if srcAt != len(src) {
		return nil, fmt.Errorf("mermaid: stream has %d trailing bytes", len(src)-srcAt)
	}
	return dst, nil
}
