...
func decodeEntropy(src []byte, capacity, depth int) (entropyResult, error) {
	if depth > maxEntropyDepth {
		return entropyResult{}, fmt.Errorf("mermaid: entropy nesting exceeds %d", maxEntropyDepth)
	}
	if len(src) < 2 {
		return entropyResult{}, fmt.Errorf("mermaid: truncated entropy header")
	}
	...
	if chunkType == 0 {
		...
		if src[0] >= 0x80 {
			...
			if len(src) < 3 {
				return entropyResult{}, fmt.Errorf("mermaid: truncated long raw entropy header")
			}
			size = int(src[0])<<16 | int(src[1])<<8 | int(src[2])
			if size&^0x3ffff != 0 {
				return entropyResult{}, fmt.Errorf("mermaid: reserved raw entropy size bits are set")
			}
			header = 3
		}
		if size > capacity || size > len(src)-header {
			return entropyResult{}, fmt.Errorf("mermaid: raw entropy size %d exceeds its bounds", size)
		}
		out := append([]byte(nil), src[header:header+size]...)
		return entropyResult{data: out, used: header + size}, nil
	}
	if chunkType >= 6 {
		return entropyResult{}, fmt.Errorf("mermaid: unsupported entropy type %d", chunkType)
	}

	var compressed, decoded, header int
	if src[0] >= 0x80 {
		if len(src) < 3 {
			return entropyResult{}, fmt.Errorf("mermaid: truncated short entropy header")
		}
		...
		if len(src) < 5 {
			return entropyResult{}, fmt.Errorf("mermaid: truncated long entropy header")
		}
		...
		if compressed >= decoded {
			return entropyResult{}, fmt.Errorf("mermaid: invalid entropy sizes %d >= %d", compressed, decoded)
		}
		header = 5
	}
	if compressed > len(src)-header || decoded > capacity {
		return entropyResult{}, fmt.Errorf("mermaid: entropy sizes %d -> %d exceed their bounds", compressed, decoded)
	}
	...
	case 1:
		err = fmt.Errorf("mermaid: entropy type 1 (tANS) is not present in the supported Palworld saves")
	case 5:
		err = fmt.Errorf("mermaid: entropy type 5 is not present in the supported Palworld saves")
	}
	if err != nil {
		return entropyResult{}, err
	}
	if len(out) != decoded {
		return entropyResult{}, fmt.Errorf("mermaid: entropy decoded %d bytes, expected %d", len(out), decoded)
	}
...
func decodeRLE(src []byte, outputSize, depth int) ([]byte, error) {
	if len(src) == 0 {
		return nil, fmt.Errorf("mermaid: empty RLE stream")
	}
	if len(src) == 1 {
	...
	if src[0] != 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("mermaid: RLE command prefix: %w", err)
		}
	...
	copyLiteral := func(count int) error {
		if count < 0 || count > back-front || count > len(out)-outAt {
			return fmt.Errorf("mermaid: invalid RLE literal length %d", count)
		}
	...
	fill := func(count int) error {
		if count < 0 || count > len(out)-outAt {
			return fmt.Errorf("mermaid: invalid RLE run length %d", count)
		}
		for i := 0; i < count; i++ {
			out[outAt+i] = rleByte
		}
	...
	for front < back {
		...
		case command >= 0x10:
			if back-front < 2 {
				return nil, fmt.Errorf("mermaid: truncated RLE command")
			}
		...
		case command == 1:
			if back-front < 2 {
				return nil, fmt.Errorf("mermaid: truncated RLE byte command")
			}
		...
		case command >= 9:
			if back-front < 2 {
				return nil, fmt.Errorf("mermaid: truncated long RLE command")
			}
		...
		default:
			if back-front < 2 {
				return nil, fmt.Errorf("mermaid: truncated long literal command")
			}
	...
	if front != back || outAt != len(out) {
		return nil, fmt.Errorf("mermaid: RLE ended at command %d/%d and output %d/%d", front, back, outAt, len(out))
	}
	return out, nil
}
...
func (r *msbReader) read(n int) (uint32, error) {
	if n < 0 || n > 24 || r.bit+n > len(r.data)*8 {
		return 0, fmt.Errorf("mermaid: truncated Huffman bitstream")
	}
	var value uint32
	for i := 0; i < n; i++ {
		at := r.bit + i
...
func readHuffmanCodebook(r *msbReader) ([2048]huffEntry, int, error) {
	...
	if second != 0 {
		return lut, 0, fmt.Errorf("mermaid: reserved Huffman codebook form")
	}
	...
	for i := range codeLength {
		...
		for {
			...
			zeros++
			if zeros > 255 {
				return lut, 0, fmt.Errorf("mermaid: invalid Golomb-Rice length")
			}
		}
		codeLength[i] = zeros
	}
	...
	for i := 0; i < numSymbols; i++ {
		...
		if codeLength[i] < 1 || codeLength[i] > 11 {
			return lut, 0, fmt.Errorf("mermaid: invalid Huffman code length %d", codeLength[i])
		}
		runningSum += delta
	}
	...
	if numSymbols == 1 {
		if len(ranges) != 1 || ranges[0].count != 1 {
			return lut, 0, fmt.Errorf("mermaid: invalid single-symbol Huffman ranges")
		}
	...
	for _, item := range ranges {
		...
		for i := 0; i < item.count; i++ {
			...
			if at < 0 || at >= len(symbols) {
				return lut, 0, fmt.Errorf("mermaid: Huffman symbol table overflow")
			}
	...
	if codeAt != numSymbols {
		return lut, 0, fmt.Errorf("mermaid: incomplete Huffman ranges")
	}
...
func convertHuffmanRanges(r *msbReader, numSymbols, fluff int, encoded []int) ([]huffRange, error) {
	...
	if fluff&1 != 0 {
		if encodedAt >= len(encoded) || encoded[encodedAt] >= 8 {
			return nil, fmt.Errorf("mermaid: invalid initial Huffman range")
		}
	...
	for i := 0; i < rangeCount; i++ {
		if encodedAt+1 >= len(encoded) || encoded[encodedAt] >= 9 || encoded[encodedAt+1] >= 8 {
			return nil, fmt.Errorf("mermaid: invalid Huffman range")
		}
	...
	if symbolAt >= 256 || used >= numSymbols || symbolAt+numSymbols-used > 256 {
		return nil, fmt.Errorf("mermaid: invalid Huffman symbol ranges")
	}
...
func makeHuffmanLUT(lut *[2048]huffEntry, prefix [12]int, symbols []byte) error {
	...
	for length := 1; length <= 11; length++ {
		...
		for i := 0; i < count; i++ {
			if start+i >= len(symbols) || slot+step > len(lut) {
				return fmt.Errorf("mermaid: invalid Huffman lookup table")
			}
	...
	if slot != len(lut) {
		return fmt.Errorf("mermaid: incomplete Huffman lookup table (%d/2048)", slot)
	}
	return nil
}
...
func (r *lsbReader) lookup(lut *[2048]huffEntry) (byte, error) {
	...
	entry := lut[index]
	if entry.length == 0 {
		return 0, fmt.Errorf("mermaid: invalid Huffman code")
	}
	r.bit += int(entry.length)
	if r.bit > len(r.data)*8+10 {
		return 0, fmt.Errorf("mermaid: Huffman stream overrun")
	}
...
func decodeHuffman(src []byte, outputSize, variant int) ([]byte, error) {
	...
	if symbols == 1 {
		if reader.byteOffset() != len(src) {
			return nil, fmt.Errorf(
				"mermaid: single-symbol Huffman codebook consumed %d/%d bytes",
				reader.byteOffset(),
				len(src),
			)
		}
	...
	if at > len(src) {
		return nil, fmt.Errorf("mermaid: Huffman codebook overrun")
	}
	src = src[at:]
	out := make([]byte, outputSize)
	...
	case 1:
		if len(src) < 3 {
			return nil, fmt.Errorf("mermaid: truncated Huffman split")
		}
		split := int(binary.LittleEndian.Uint16(src))
		src = src[2:]
		if split > len(src) {
			return nil, fmt.Errorf("mermaid: invalid Huffman split %d", split)
		}
	...
	case 2:
		if len(src) < 6 {
			return nil, fmt.Errorf("mermaid: truncated six-stream Huffman split")
		}
		middle := int(src[0]) | int(src[1])<<8 | int(src[2])<<16
		src = src[3:]
		if middle > len(src) || middle < 2 {
			return nil, fmt.Errorf("mermaid: invalid Huffman middle split %d", middle)
		}
		...
		if leftSplit > len(leftHalf) || len(rightHalf) < 2 {
			return nil, fmt.Errorf("mermaid: invalid Huffman left split")
		}
		rightSplit := int(binary.LittleEndian.Uint16(rightHalf))
		rightHalf = rightHalf[2:]
		if rightSplit > len(rightHalf) {
			return nil, fmt.Errorf("mermaid: invalid Huffman right split")
		}
		half := (len(out) + 1) >> 1
	...
	default:
		return nil, fmt.Errorf("mermaid: unsupported Huffman variant %d", variant)
	}
	return out, nil
}
...
func decodeHuffmanStreams(dst, first, shared []byte, lut *[2048]huffEntry) error {
	...
	if a.bytesUsed() != len(first) {
		return fmt.Errorf("mermaid: Huffman first stream consumed %d/%d bytes", a.bytesUsed(), len(first))
	}
	if b.bytesUsed()+c.bytesUsed() != len(shared) {
		return fmt.Errorf("mermaid: Huffman shared streams consumed %d+%d/%d bytes", b.bytesUsed(), c.bytesUsed(), len(shared))
	}
	return nil
}
